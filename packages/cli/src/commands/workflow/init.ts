...
export async function workflowInitCommand(name?: string): Promise<void> {
	Logger.header('Create New Workflow');
	...
	try {
		...
		const answers = await inquirer.prompt([
			...
			},
			{
				...
				validate: (input: number) => {
					if (input < 0) {
						return 'Trial days must be 0 or greater';
					}
					if (input > 365) {
						return 'Trial days must be 365 or less';
					}
					return true;
				},
			},
		...
		const spinner = Logger.spinner(`Creating workflow project "${projectName}"...`);
