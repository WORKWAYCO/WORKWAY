 ...
 * Uses the `bd` CLI for operations to avoid conflicts with daemon sync.
 *
 * Why CLI over direct file writes:
 * - bd CLI uses SQLite as source of truth
 * - bd daemon syncs SQLite → JSONL periodically
 * - Direct JSONL writes get overwritten by daemon
...
export async function createIssue(
  ...
  // Parse the issue ID from output (e.g., "Created issue: Cloudflare-abc")
 ...
 * Uses bd list with manual filtering since bd ready has a limit.
 */
export async function getReadyIssues(cwd?: string): Promise<BeadsIssue[]> {
  // Get all issues
  const issues = await readAllIssues(cwd);

  // Get blocked issues via bd blocked command
  let blockedIds: Set<string>;
  try {
    const blockedOutput = await bd('blocked --json', cwd);
    const blockedIssues = JSON.parse(blockedOutput) as Array<{ id: string }>;
    blockedIds = new Set(blockedIssues.map(i => i.id));
  } catch {
    // If blocked command fails, assume nothing is blocked
    blockedIds = new Set();
  }

  // Filter to open, unblocked issues sorted by priority
  return issues
    .filter((issue) => issue.status === 'open' && !blockedIds.has(issue.id))
    .sort((a, b) => (a.priority || 2) - (b.priority || 2));
}

/**
 * Get harness-labeled issues ready for work.
 */
export async function getHarnessReadyIssues(harnessId: string, cwd?: string): Promise<BeadsIssue[]> {
  const readyIssues = await getReadyIssues(cwd);
