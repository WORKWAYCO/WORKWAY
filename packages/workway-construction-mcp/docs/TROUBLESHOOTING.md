...
**Start here:** Use `workway_diagnose` tool with your symptom:
...
**Symptoms:**
- `workway_deploy` returns validation errors
...
2. **Common validation failures:**
   ...
   **Missing actions:**
   ...
   ```
   **Fix:** Add actions with `workway_add_action`
   ...
   **Missing trigger config:**
   ...
   ```
   **Fix:** Configure trigger with `workway_configure_trigger`
...
- `workway_diagnose` with `symptom: "webhook_not_firing"`
...
1. **Get execution details:**
   ```json
   {
     "tool": "workway_observe_execution",
     "params": {
...
**Prevention:**
- Test workflows with `workway_test` before deploying
...
- `workway_diagnose` with `symptom: "ai_output_wrong"`
...
4. **Verify solution:**
   - Test with `workway_test` if applicable
   - Check execution logs
   - Monitor for recurrence

5. **If still stuck:**
   - Use `workway_get_unstuck` for guidance
   - Review execution traces with `workway_observe_execution`
...
- [ ] Always test workflows with `workway_test` before deploying
