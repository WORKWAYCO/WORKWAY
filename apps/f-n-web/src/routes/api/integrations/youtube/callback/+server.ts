...
export const GET: RequestHandler = async ({ url, platform }) => {
	...
	const clientSecret = platform.env.YOUTUBE_CLIENT_SECRET;
	...
	// Verify state
	const stateData = await SESSIONS.get(`youtube_oauth_state:${state}`, 'json') as { userId: string; email?: string } | null;
	...
	// Clean up state
	await SESSIONS.delete(`youtube_oauth_state:${state}`);
	...
	try {
		...
		if (existing) {
			...
			// Create new
			await DB.prepare(
				`INSERT INTO connected_accounts (id, user_id, provider, access_token, refresh_token, expires_at)
				 VALUES (?, ?, ?, ?, ?, ?)`
			)
				.bind(
					crypto.randomUUID(),
		...
		console.error('Database error saving YouTube connection:', dbError);
		throw redirect(302, '/dashboard?error=database_error');
	}

	throw redirect(302, '/dashboard?success=youtube_connected');
};
