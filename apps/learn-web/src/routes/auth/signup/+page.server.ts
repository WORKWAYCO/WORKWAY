...
export const actions: Actions = {
	default: async ({ request, cookies, platform }) => {
		...
		try {
			...
			if (!response.ok) {
				const errorData = await response.json().catch(() => ({ message: 'Signup failed' })) as { message?: string };
				return fail(response.status, { error: errorData.message || 'Unable to create account' });
			}
			...
			return fail(500, { error: 'Unable to connect to authentication service' });
		}
