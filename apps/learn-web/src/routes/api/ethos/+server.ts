...
export const PUT: RequestHandler = async ({ request, locals, platform }) => {
	...
	let body: { ethos: Record<string, { category: string; content: string; context?: string }> };
	try {
	...
	// Validate categories
	const validCategories = ['zuhandenheit', 'outcome_focus', 'simplicity', 'resilience', 'honesty'];
		...
		for (const [category, principle] of Object.entries(body.ethos)) {
			await db
				...
				.bind(
					...
					category,
					principle.content,
					principle.context || DEFAULT_ETHOS[category as keyof typeof DEFAULT_ETHOS]?.context || null
				)
				.run();
		}
