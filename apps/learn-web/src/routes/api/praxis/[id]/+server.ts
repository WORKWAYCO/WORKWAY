...
const praxisValidators: Record<
  string,
  (submission: PraxisSubmission) => PraxisResult
> = {
  ...
  triggers: (submission) => {
    ...
    return {
      ...
      nextSteps: [
        "Consider which events in your work could trigger automations",
      ],
    };
  },
  ...
  "working-with-integrations": (submission) => {
    ...
    return {
      ...
      nextSteps: [
        "Add error isolation so one failure doesn't break everything",
      ],
    };
  },
  ...
  "workers-ai": (submission) => {
    ...
    return {
      ...
      nextSteps: [
        "Experiment with different AI models and temperature settings",
      ],
    };
  },
